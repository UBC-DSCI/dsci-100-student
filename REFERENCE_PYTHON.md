...
For example, the function reference for a pandas data frame is `pd.DataFrame(data)`,
...
| Name                        | Description                                                                   |
|-----------------------------|-------------------------------------------------------------------------------|
| DataFrame(data)             | A two-dimensional, heterogeneous tabular data structure with labeled axes     |
| Series(data)                | A one-dimensional labeled array capable of holding data of various types      |
| crosstab(df)                | Compute a cross-tabulation of two (or more) factors, e.g. a confusion matrix. |
| concat(objs)                | Concatenate pandas objects (dfs, series) along a particular axis              |
...
| groupby(by)                         | Group rows together if share value in the specified column                    |
| contains(pat)                       | For searching for a str within the values of a df column                      |
| isin(values)                        | Determine whether a value is present in a column                              |
...
| Name                             | Description                                                                                                |
|----------------------------------|------------------------------------------------------------------------------------------------------------|
| Chart(data)                      | Specity the data used for the chart                                                                        |
| Color, X, Y, ...                 | Helpers for modifying the corresponding encoding, eg. adding a title                                       |
| axis(tickCount)                  | Modify the axis format of a single axis                                                                    |
| configure_axis(axis)             | Modify the axis format of all axes                                                                         |
...
| facet('column_name', columns)    | Create multiple views of a dataset where each panel contains a different subset                            |
...
| scale(zero)                      | Modify scale properties for encoding channels in the chart                                                 |
...
| make_column_transformer(transformers)      | Function for creating a composite transformer for feature preprocessing.                                     |
...
| np.array(object)                     | Create a NumPy array from a list or other iterable                                                                      |
