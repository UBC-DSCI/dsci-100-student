...
test_1.2 <- function(){
    ...
    test_that('cheapest does not contain the correct data.', {
        ...
        expect_equal(digest(int_round(unlist(select(cheapest, -region)), 2)), 'bd15e8575c605a44f323d2c67cbfd7f1') # we hid the answer to the test here so you can't see it, but we can still run the test
        })
print("Success!")
    }
