...
test_1.4 <- function(){
    ...
    test_that('the column names of sample_estimates should be replicate and mean_grade', {
        expect_equal(digest(paste(sort(colnames(sample_estimates)), collapse = "")), 'e0c8c7481aedaea09cc003a0ef289019')
    })
    print("Success!")
}
...
test_1.5 <- function(){
    ...
    test_that('mean_grade should be on the x-axis.', {
        expect_true("mean_grade" == rlang::get_expr(properties$x))
    })
   ...
   test_that('Labels on the x axis should be descriptive. The plot should have a descriptive title.', {
        expect_false((labels$x) == 'mean_grade')
...
test_1.8 <- function(){
    ...
    test_that('coffee_sample_estimates_5 does not contain a column called coffee_mean_cups_5', {
        expect_true('coffee_mean_cups_5' %in% colnames(coffee_sample_estimates_5))
    })
    test_that('coffee_sample_estimates_5 does not contain the correct data', {
        expect_equal(dim(coffee_sample_estimates_5), c(1500, 2))
        expect_equal(digest(int_round(sum(coffee_sample_estimates_5$coffee_mean_cups_5), 2)), '1c69dbae0a048e8a95be34cbdd4ccb9f')
    })
    test_that('coffee_mean_cups_5 should be on the x-axis.', {
        expect_true("coffee_mean_cups_5" == rlang::get_expr(properties$x))
    })
   ...
   test_that('Labels on the x axis should be descriptive. The plot should have a descriptive title.', {
        expect_false((labels$x) == 'coffee_mean_cups_5')
        expect_false(is.null(labels$title))
    })
    test_that('size and rep values are incorrect', {
...
test_2.0 <- function(){
    ...
    test_that('coffee_sample_estimates_30 does not contain a column called coffee_mean_cups_30', {
        expect_true('coffee_mean_cups_30' %in% colnames(coffee_sample_estimates_30))
    })
    test_that('coffee_sample_estimates_30 does not contain the correct data', {
        expect_equal(dim(coffee_sample_estimates_5), c(1500, 2))
        expect_equal(digest(int_round(sum(coffee_sample_estimates_30$coffee_mean_cups_30), 2)), 'c7c6210820892e507d0173ba167d416f')
    })
    test_that('coffee_mean_cups_30 should be on the x-axis.', {
        expect_true("coffee_mean_cups_30" == rlang::get_expr(properties$x))
    })
   ...
   test_that('Labels on the x axis should be descriptive. The plot should have a descriptive title.', {
        expect_false((labels$x) == 'coffee_mean_cups_30')
        expect_false(is.null(labels$title))
    })
    test_that('size and rep values are incorrect', {
