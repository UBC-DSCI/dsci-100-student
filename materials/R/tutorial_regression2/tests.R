...
test_1.3 <- function() {
    test_that('credit_pairplot should be a pairwise plot matrix.', {
        expect_true('ggmatrix' %in% c(class(credit_pairplot)))
    })
    test_that('credit_pairplot should be using data from the credit data frame.', {
        expect_equal(int_round(nrow(credit_pairplot$data), 0), 239)
    })
    test_that('credit_pairplot should be using the Balance, Income, Rating, Limit columns.', {
        expect_equal(int_round(ncol(credit_pairplot$data), 0), 3)
    })
    print("Success!")
}
