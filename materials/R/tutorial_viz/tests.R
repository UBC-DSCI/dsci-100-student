...
test_2 <- function(){
    properties <- c(bar_plot_palette$layers[[1]]$mapping, bar_plot_palette$mapping)
    labels <- bar_plot_palette$labels
    test_that('Did not create a plot named bar_plot_palette', {
        expect_true(exists("bar_plot_palette")) 
        })
    test_that('sex should be on the x-axis.', {
        expect_true("sex" == rlang::get_expr(properties$x))
        })
    test_that('the region variable should be used to colour fill the bars.', {
        expect_true("smoker" == rlang::get_expr(properties$fill))
        })
    test_that('bar_plot_palette should be a bar plot.', {
        expect_true("GeomBar" %in% c(class(bar_plot_palette$layers[[1]]$geom)))
        })
    test_that('bar_plot_palette position should be fill', {
        expect_true("PositionFill" %in% class(bar_plot_palette$layers[[1]]$position))
        })
    test_that('Labels on the axes and legend should be descriptive and human readable.', {
        expect_false((labels$x) == 'sex')
        expect_false((labels$fill) == 'smoker')
        })
    test_that('bar_plot_palette should not use the default color palette.', {
        expect_false(identical(scales::hue_pal()(2), unique(ggplot_build(bar_plot_palette)$data[[1]]$fill)))
        })
print("Success!")
    }
