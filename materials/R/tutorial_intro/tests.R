...
test_1.10 <- function(){
    test_that('Did not create a plot named racetime_vs_age', {
        expect_true(exists("racetime_vs_age")) 
        })
    test_that('Did not create a data frame named marathon_small_mins', {
        expect_true(exists("marathon_small_mins")) 
    })
    
    properties <- c(racetime_vs_age$layers[[1]]$mapping, racetime_vs_age$mapping)
    ...
    test_that('age_vs_time should be a scatter plot.', {
        expect_true("GeomPoint" %in% c(class(racetime_vs_age$layers[[1]]$geom)))
        })
    test_that('Labels on the axes should be descriptive and human readable.', {
        expect_false(racetime_vs_age$labels$y == 'km5_time_minutes')
        expect_false(racetime_vs_age$labels$x == 'age') # removed since 'age' is human-readable
        })
print("Success!")
    }
