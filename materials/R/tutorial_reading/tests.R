...
test_1.3.2 <- function() {
  ...
  test_that("Columns in reduced_happy_df contain incorrect values.", {
    expect_equal(digest(int_round(sum(reduced_happy_df$positive_affect_scaled, na.rm = TRUE), 2)), '0a667e2dfc8faca592dde223c1ec3dec')
    expect_equal(digest(int_round(sum(reduced_happy_df$healthy_life_expectancy_at_birth, na.rm = TRUE), 2)), '90a8884408be62b74a0558ab7fb40db0')
  })
  print("Success!")
}
...
test_1.4 <- function() {
  ...
  test_that("healthy_life_expectancy_at_birth should be on the x-axis.", {
    expect_true("healthy_life_expectancy_at_birth" == rlang::get_expr(properties$x))
  })
  test_that("positive_affect_scaled should be on the y-axis.", {
    expect_true("positive_affect_scaled" == rlang::get_expr(properties$y))
  })
  test_that("happy_plot should be a scatter plot.", {
    expect_true("GeomPoint" %in% c(class(happy_plot$layers[[1]]$geom)))
  })
  test_that("Labels on the axes should be descriptive and human readable.", {
    expect_false(happy_plot$labels$y == "positive_affect_scaled")
    expect_false(happy_plot$labels$x == "healthy_life_expectancy_at_birth")
  })
  print("Success!")
}
...
test_2.3 <- function() {
  ...
  test_that("date_time should be on the x-axis.", {
    expect_true("date_time" == rlang::get_expr(properties$x))
  })
  test_that("total_snow_cm should be on the y-axis.", {
    expect_true("total_snow_cm" == rlang::get_expr(properties$y))
  })
  test_that("whistler_2018_plot should be a line plot.", {
    expect_true("GeomLine" %in% c(class(whistler_2018_plot$layers[[1]]$geom)))
  })
  test_that("Labels on the axes should be descriptive and human readable.", {
    expect_false(whistler_2018_plot$labels$y == "total_snow_cm")
    expect_false(whistler_2018_plot$labels$x == "date_time")
  })
  print("Success!")
}
